...
function showCustomerUpdateRelatedRecordLinks() {
    chrome.tabs.query(
        { currentWindow: true, active: true },
        function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, { method: "runFunction", myVars: "showCustomerUpdateRelatedRecordLinks();", funtion() { } });
        });
}
...
function showFormSections() {
    chrome.tabs.query(
        { currentWindow: true, active: true },
        function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, { method: "runFunction", myVars: "showFormSections();", funtion() { } });
        });
}
